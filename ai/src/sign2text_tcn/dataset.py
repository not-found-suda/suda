...
TARGET_SAMPLES_PER_CLASS = int(os.environ.get("V6_TARGET_SAMPLES_PER_CLASS", "120"))
USE_AUGMENTATION = os.environ.get("V6_USE_AUGMENTATION", "1") == "1"
NORMALIZE_MODE = os.environ.get("V6_NORMALIZE_MODE", "none").strip().lower()
AUGMENT_SEED = 2026
NOISE_STD = float(os.environ.get("V6_AUG_NOISE_STD", "0.005"))
TIME_SCALE_RANGE = (
    float(os.environ.get("V6_AUG_TIME_SCALE_MIN", "0.95")),
    float(os.environ.get("V6_AUG_TIME_SCALE_MAX", "1.05")),
)
...
PRIORITY_FILE_KEYWORDS = tuple(
    ...
    for keyword in os.environ.get(
        "V6_PRIORITY_FILE_KEYWORDS",
        os.environ.get("V5_PRIORITY_FILE_KEYWORDS", "_FRONT,_LEFT,_RIGHT"),
    ).split(",")
    if keyword.strip()
)
INPUT_DIM = 332
...
class KSLDatasetV6(Dataset):
    ...
    def normalize_features(self, data):
        ...
        if NORMALIZE_MODE in ("none", "raw", "off"):
            return data
        if NORMALIZE_MODE in ("sequence", "sequence_zscore", "zscore"):
            mean = data.mean(axis=0, keepdims=True)
            std = data.std(axis=0, keepdims=True)
            return ((data - mean) / np.maximum(std, 1e-6)).astype(np.float32)
        if NORMALIZE_MODE != "shoulder":
            raise ValueError(
                f"Unsupported V6_NORMALIZE_MODE={NORMALIZE_MODE}. "
                "Use one of: none, sequence_zscore, shoulder."
            )
...
def create_dataloaders_v6(
    ...
    train_dataset = KSLDatasetV6(X_train_items, y_train_items, max_len=max_len, input_dim=input_dim)
    val_dataset = KSLDatasetV6(X_val, y_val, max_len=max_len, input_dim=input_dim)
