...
def resolve_model_dir(model_dir=None):
    ...
    if numbered_dirs:
        return sorted(numbered_dirs, key=lambda path: int(path.name.rsplit("_", 1)[-1]))[-1]
...
def find_latest_numbered_pair(model_dir):
    ...
    for model_prefix, label_prefix, config_prefix in (
        ...
        for model_path in model_dir.glob(f"{model_prefix}*.pt"):
            ...
            label_map_path = model_dir / f"{label_prefix}{suffix}.json"
            if label_map_path.exists():
                pairs.append((int(suffix), model_path, label_map_path))
                continue

            config_path = model_dir / f"{config_prefix}{suffix}.json"
...
def load_model(model_path, num_classes):
    config = load_train_config(model_path)
    model = build_ksl_model_v6(
