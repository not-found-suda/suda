...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a TFLite CTC labeler and greedy-decode word tokens.")
...
def main() -> None:
    ...
    blank_id = int(label_map.get("blank_id", 0))
    input_shape = list(label_map["input_shape"])
    ...
    for row in rows:
        ...
        if int(features.shape[1]) != input_dim:
            raise ValueError(f"Feature dim mismatch for {row['video_name']}: {features.shape[1]} != {input_dim}")
        model_input = resample_features(features, sequence_length)[None, :, :]
        interpreter.set_tensor(feature_input["index"], model_input.astype(np.float32))
        interpreter.invoke()
        logits = interpreter.get_tensor(output["index"])[0]
        predicted_ids = np.argmax(logits, axis=-1)
        predicted_words = greedy_decode_ids(predicted_ids, id_to_label, blank_id=blank_id)
