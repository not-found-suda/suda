...
from decode_sequence_labels import collapse_ids
from train_sequence_labeler import PAD_LABEL_ID, SequenceLabeler


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a trained sequence labeler and collapse predictions to words.")
    ...
    parser.add_argument("--report-labels", action="store_true", help="Print per-label precision/recall and top errors.")
...
def main() -> None:
    ...
    with torch.no_grad():
        for row in rows:
            ...
            labels = np.load(row["label_path"]).astype(np.int64)
            ...
            predicted_ids = probabilities.argmax(dim=-1).squeeze(0).cpu().numpy()
            predicted_confidences = probabilities.max(dim=-1).values.squeeze(0).cpu().numpy()
            predicted_words = collapse_ids(
                predicted_ids,
                id_to_label,
                args.min_run,
            ...
            mask = labels != PAD_LABEL_ID
            if mask.any():
                frame_correct += int((predicted_ids[mask] == labels[mask]).sum())
    ...
    print(f"summary files={len(rows)} sequence_acc={sequence_acc:.4f} wer={wer:.4f} frame_acc={frame_acc:.4f}")
